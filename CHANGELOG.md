...
- **`ExRatatui.Burrito` — runtime support for wrapped TUIs.** `main/3` is the entry-point protocol the scaffolded CLI delegates to: a no-op outside a burrito-wrapped binary (detected via the `__BURRITO` env var burrito's wrapper sets, so consumer `mix test`/`iex -S mix` sessions never boot the TUI), `--version` anywhere in argv for TTY-less smoke tests (forcing the NIF `dlopen` first, so a NIF/host mismatch fails loudly), and exit-code mapping for every failure path — TUI crash, failed start, or a raise in the entry point all report on stderr and stop the VM with exit 1 instead of hanging the wrapper on an idle BEAM. Living in the library, protocol fixes reach existing consumers with a dep upgrade instead of freezing in generated code. `verify_linux_nif/1` is a release step wired between `:assemble` and `&Burrito.wrap/1` that fails the linux build with the exact fix when the assembled NIF is glibc — the forgotten-`TARGET_ABI=musl` mistake becomes a build error rather than a shipped binary that hangs at NIF load on every end-user machine.

- **`mix ex_ratatui.gen.burrito` — Igniter-based generator.** Patches a consumer's `mix.exs` (adds `{:burrito, "~> 1.5"}`, merges the four standard targets plus the `verify_linux_nif/1` step into `releases:` without disturbing existing entries — a non-literal value it cannot patch, or an existing entry for the app, produces a warning instead), drops a thin CLI shim (`use Task`, keyed on the CLI module so the wiring stays idempotent) delegating to `ExRatatui.Burrito.main/3`, and adds a `.mise.toml` pinning `zig 0.15.2`. `--ci github` additionally scaffolds a `.github/workflows/release.yml` that builds + publishes binaries to GitHub Releases on tag push. Igniter is declared `optional: true`; projects that never run the generator pay nothing for it, and invoking the task without Igniter installed prints an install hint and exits cleanly. `examples/burrito_demo`'s CLI and `.mise.toml` are pinned byte-for-byte to the generator's output by tests, so the demo cannot drift from what consumers scaffold.

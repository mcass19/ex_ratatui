...
- **`mix ex_ratatui.gen.burrito` — Igniter-based generator.** Patches a consumer's `mix.exs` (adds `{:burrito, "~> 1.5"}`, merges the four standard targets into `releases:` without disturbing existing entries — a non-literal value it cannot patch produces a warning instead), drops a CLI module with `main/1` + `--version` smoke entry, and adds a `.mise.toml` pinning `zig 0.15.2`. The generated CLI only boots the TUI inside the wrapped binary (`Burrito.Util.running_standalone?/0`), so the same supervised Task is a no-op under the consumer's `mix test` and `iex -S mix`, and it exits non-zero (instead of hanging the wrapper) when the TUI crashes or the terminal cannot initialize. `--ci github` additionally scaffolds a `.github/workflows/release.yml` that builds + publishes binaries to GitHub Releases on tag push. Igniter is declared `optional: true`; projects that never run the generator pay nothing for it, and invoking the task without Igniter installed prints an install hint and exits cleanly. `examples/burrito_demo`'s CLI and `.mise.toml` are pinned byte-for-byte to the generator's output by tests, so the demo cannot drift from what consumers scaffold.
